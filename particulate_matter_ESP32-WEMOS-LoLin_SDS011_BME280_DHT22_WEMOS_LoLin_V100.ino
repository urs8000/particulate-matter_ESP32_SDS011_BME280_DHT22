 ...
 * Global variables use 10508 bytes (3%) of dynamic memory, leaving 284404 bytes for local variables. Maximum is 294912 bytes.
 *
 * * * *  D O I T  * * *                  Board: ESP32 Dev Module
 * ESP32  ---->  HOPR-RFM95 modul
 *  D5    ->    nSS
 *  D18   ->    SCK
 *  D19   ->    MISO
 *  D23   ->    MOSI
 ...
 * * * *  W E M O S  * * *                Board: "WEMOS" WiFi&Bluetooth Battery
 * ESP32      --->  HOPR-RFM95 modul
 *  GND       ->    Top Right
 *  3V3       ->    Bottom Right
 *  SS/5/LED  ->    nSS
 ...
 ******************************************************************************************************************************/
 ...
 // Temp BME:   2 bytes    multiplied by 100        overwrites Temp DHT22  if both available
 // Humid BME:  2 bytes    multiplied by 100        overwrites Hum  DHT22  if both available
 // Press BME:  2 bytes    no nachkommastellen      000 if only DHT22
 // 
...
#define BME280    0                        // use BME280        address 0x76
...
#include <Adafruit_BME280.h>               // change I2C address of the sensor  0x77 --> 0x76
